...
def into_datetime(string):
    ...
    one = datetime.date.fromisoformat(f'{two_dates[1].split(".")[2].replace("г", "")}{two_dates[0].split(".")[1]}{two_dates[0].split(".")[0]}'.replace(" ", ""))
    two = datetime.date.fromisoformat(f'{"".join([i.replace("г", "") for i in two_dates[1].split(".")[::-1] ])}'.replace("xlsx", ""))
    return one, two
