...
def into_datetime(string):
	...
	print(f'{two_dates[1].split(".")[2].replace("г", "")}{two_dates[0].split(".")[1]}{two_dates[0].split(".")[0]}')
	one = datetime.date.fromisoformat(
		f'{two_dates[1].split(".")[2].replace("г", "")}{two_dates[0].split(".")[1]}{two_dates[0].split(".")[0]}'.replace(
			" ", ""))
	two = datetime.date.fromisoformat(
		f'{"".join([i.replace("г", "") for i in two_dates[1].split(".")[::-1]])}'.replace("xlsx", ""))
	return one, two
...
def get_schedule(day, course, group, redirection=False):  # day = "Today", "Tomorrow", либо "Weekly"
	...
	# Время со смещением
	new_time = now + offset
	new_time1 = datetime.date.fromisoformat(str(new_time).split()[0])
	perm_time = new_time.strftime('%d.%m.%Y')
	path, filename = 'downloaded_xlsx', ""
	for each in os.listdir(path):  # Перебор файлов
		if f'Расписание {course} курс на' in each:
			...
			date1, date2 = into_datetime(dates)
			if date1 <= new_time1 <= date2:
				filename = each
				print("Файл найден")
			elif not (date1 <= new_time1 <= date2) and redirection == True:
	...
	if filename != "":
		...
		if day == "Today" or day == "Tomorrow":
			...
			for row in sheet.iter_rows(min_row=7):
				nums = []
				for cell in row:
					if ((need_group.column <= cell.column <= (need_group.column + 4)) or (cell.column in (2, 4))) and (
							cell.value != None and cell.value != [] and (need_row + 11) >= cell.row >= (need_row)):
			...
			for i in range(0, 5):
				...
				for row in sheet.iter_rows(min_row=week_days[i][0], max_row=week_days[i][1]):
					nums = []
					for cell in row:
						if ((need_group.column <= cell.column <= (need_group.column + 4)) or (
								cell.column in (2, 4))) and not (cell.value in ([], None)):
							nums.append(f'{str(cell.value)}{')' * (cell.column == 2)}')
					if nums != []:
						schedule.append(nums)
...
"""p = get_schedule("Tomorrow", "3-4", 'УП-922')
for i in p:
	print(str(i))
	for j in p[i]:
		print(*j)"""
